...
import { sendWaitlistEmail } from "../lib/waitlist";
  ...
  head: () => ({
    meta: [
      { title: "Wachtlijst & Contact — HIGHBALL CLUB" },
      {
        name: "description",
        content:
          "Meld je aan voor de wachtlijst en proef als eerste. Of neem contact op met Highball Club.",
      },
    ],
    ...
    links: [
      {
        rel: "canonical",
        href: "https://highball.pages.dev/wachtlijst",
      },
    ],
  }),
...
function WachtlijstPage() {
  ...
  const submitWaitlist = useServerFn(sendWaitlistEmail);
  ...
  const onSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      console.error("Wachtlijst verzenden mislukt:", err);
          ...
          {submitted ? (
            ...
            <form onSubmit={onSubmit} className="space-y-5">
              ...
              <button
                type="submit"
                disabled={pending}
                className="btn-premium w-full rounded-full py-3.5 text-sm font-medium disabled:opacity-60 disabled:cursor-not-allowed"
              >
                {pending ? "Bezig met verzenden…" : "Meld me aan"}
              </button>
            </form>
          )}
        </div>
